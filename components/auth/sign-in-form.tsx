...
export default function SignInForm({ configured }: { configured: boolean }) {
  ...
  return <form className="signin-form" onSubmit={submit}>
    ...
    <label><span>Password</span><div className="signin-input"><LockKeyhole size={16}/><input type={showPassword ? "text" : "password"} autoComplete="current-password" required maxLength={128} value={password} onChange={event=>setPassword(event.target.value)} placeholder="Enter your password" disabled={!configured}/><button type="button" onClick={()=>setShowPassword(!showPassword)} aria-label={showPassword ? "Hide password" : "Show password"} disabled={!configured}>{showPassword ? <EyeOff size={15}/> : <Eye size={15}/>}</button></div></label>
